...
Node *sort2(Node *head)
{
    ...
    while (curr != NULL)
    {
        ...
        {
            Node *temp = curr; // one wali node separate ki
            curr = curr->next;
            temp->next = NULL;

            // append 1 node in oneHead LL
            oneTail->next = temp;
            oneTail = temp;
        }
        else if (curr->data == 2)
        {
            Node *temp = curr; // two wali node separate ki
            curr = curr->next;
            temp->next = NULL;

            // append 2 node in twoHead LL
            twoTail->next = temp;
            twoTail = temp;
        }
    }
...
Node *solve(Node *&head1, Node *head2)
{
    ...
    while (head2 != NULL)   // jb head2 bada hoga
    {
