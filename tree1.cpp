...
{
    // base case   means leaf node pe hum NULL lga rhe hai 
    if (data == -1)
    {
        return NULL;
    }
...
// 2 level order traversal in tree
void levelOrderTraversal(Node *root)
{
    ...
    q.push(root);
    q.push(NULL); // change here for new line coz for every new level we have to print on new line thats it
    while (!q.empty())
    {
        Node *temp = q.front();
        q.pop();
        ...
        else
        {
            ...
            if (temp->left)  //agr left exists krta hai to
            {
                q.push(temp->left);
            }

            if (temp->right)   //agr right exists krta hai to
            {
                q.push(temp->right);
            }
        }
