...
#include <limits>
#include <algorithm>
// n trains and 1 plaftform problem arrival time and departure time
// algo
...
int main()
{
  ...
  int arr[] = {4, 3, 2, 6};
  int n = 4;
  // mean heap top pe hmesha min elem hoga
  // qki min min add krke roap krenge
  ...
  int cost = 0;
  while (pq.size() != 1) // jb tk min heap ki size 1 nhi ho jati
  {
    int first = pq.top();
    pq.pop();
    int second = pq.top();
...
//need to study this comapre  wale function
  bool cmp(pair<int, int> a, pair<int, int> b)
  {
